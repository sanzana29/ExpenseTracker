  ...
  if (err) {
    ...
    db.query(
      ...
      CREATE TABLE IF NOT EXISTS expense (
        ...
        edate DATE,
        ecategory VARCHAR(255),
        ereference VARCHAR(255),
        FOREIGN KEY (user_id) REFERENCES user(id) ON DELETE CASCADE
      )
    `,
      (err) => {
...
app.put("/api/update-password", async (req, res) => {
  ...
  db.query(sqlSelect, [req.session.userId], async (err, result) => {
    ...
    if (!match) {
      logger.warn(
        "Incorrect old password provided for user:",
        req.session.userId
      ); // Log warning for incorrect password
    ...
    try {
      ...
      logger.error(
        "Error hashing new password for user:",
        req.session.userId,
        error
      ); // Log error during hashing
      return res.status(500).json({ message: "Server error" });
    }
  });
});
